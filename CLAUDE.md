...
src/
  ...
  pages/          # route components by feature
    Task/  Sales/  Support/  Settings/  Reports/  Master/  auth/
      Sales/Quotations/  # builder + PDF templates (spec 3, 2026-09-18)
...
- `pages/Sales/Quotations/QuotationBuilder.jsx` is lazy and is the only route whose import graph reaches `@react-pdf/renderer` (`templates/*`, `pdf/fonts.js`, `pdf/parts.jsx`, `usePdfPreview.js`) — the preview **is** the PDF (`usePdfPreview`), so there is no second renderer to drift. `QuotationList.jsx`/`LeadQuotations.jsx` get their status labels from `quoteStatus.js` instead of the builder's chain specifically to stay out of that graph — a static import of one constant from the wrong neighbour already dragged the whole PDF engine into the list's bundle once (task 17, 2026-09-19); `quoteStatus.js` exists because of that.
